...
public class PathFinderSquare implements PathFinder {
    EscapeBoard board;

    PathFinderSquare(EscapeBoard board) {
        this.board = board;
    }
        ...
        while (q.size() > 0) {
            ...
            while (adj.size() > 0) {
                ...
                if (next.equals(to)) {
                    ...
                    return true;
                } else if (openCoordinate(false, piece, next)) {
                    q.add(next);
                }
    ...
    public boolean findPathSingleDirection(EscapePieceImpl piece, Coordinate from, Coordinate to) {
        ...
        while (q.size() > 0) {
            ...
            if (next.equals(to) && (openCoordinate(true, piece, next))) {
                return true;
            }
    ...
    public int[][] getDirections(EscapePiece.MovementPattern movementPattern) {
        DirectionFinder directionFinder = new DirectionFinder(Coordinate.CoordinateType.SQUARE, movementPattern);
        return directionFinder.getDirections(Coordinate.CoordinateType.SQUARE, movementPattern);
    ...
    public int[] getLinearDirection(Coordinate to, Coordinate from) {
        DirectionFinder directionFinder = new DirectionFinder(Coordinate.CoordinateType.SQUARE, EscapePiece.MovementPattern.LINEAR);
    return directionFinder.getDirectionsLinear(Coordinate.CoordinateType.SQUARE, to, from);
    }
